...
### LC.2008. 出租车的最大盈利 -2023-12-08

```java
    // lc.2008. 出租车的最大盈利
    // 使用dp 记录到达某个位置的最大盈利
    // dp[i] = dp[startj] + endj - startj + tipj 其中j为在i点下车的乘客 及 endj = i
    // 若该点没有乘客下车 那么dp[i] = dp[i - 1]
    public long maxTaxiEarnings(int n, int[][] rides) {
        long[] dp = new long[n + 1];
        Map<Integer, List<int[]>> targetMap = new HashMap<Integer, List<int[]>>();
        for (int[] ride : rides) {
            if (!targetMap.containsKey(ride[1]))
                targetMap.put(ride[1], new ArrayList<>());
            targetMap.get(ride[1]).add(ride);
        }
        for (int i = 1; i <= n; i++) {
            dp[i] = dp[i - 1];
            if (targetMap.containsKey(i)){
                for (int[] ride: targetMap.get(i)){
...
### LC.2048. 下一个更大的数值平衡数 -2023-12-09
...
```java
    ...
    // dp 计算到某点最小的体力消耗路径 但没有递推顺序 是可以走回退路线的 而不是左上->右下
    // 采用Dijkstra
    public int minimumEffortPath(int[][] heights) {
        ...
        while (!pq.isEmpty()) {
            ...
            seen[id] = true;
            // 扩充访问的介节点集 更新访问节点集到未访问节点集的距离
            for (int i = 0; i < 4; ++i) {
                int nx = x + dirs[i][0];
                int ny = y + dirs[i][1];
                if (nx >= 0 && nx < m && ny >= 0 && ny < n && Math.max(d, Math.abs(heights[x][y] - heights[nx][ny])) < dist[nx * n + ny]) {
                    // dist[nx * n + ny] 表示 起始点到该点的距离 将x,y加入访问节点集时需要更新未访问节点的路径长度
                    // Math.max(d, Math.abs(heights[x][y] - heights[nx][ny])) 是路径长度的定义
...
###  LC.2454. 下一个更大元素 IV -2023-12-12

````java
    // lc.2454. 下一个更大元素 IV
    // 使用单调栈来维护一个递减序列 由于寻找的是下一个更大元素
    // 故当从第一个单调栈中弹出的时候还需要进入下一个单调栈中 等待弹出的时候
    // stack1 表示第一轮的单调栈 stack2表示第二轮的单调栈
    // 由于stack1被淘汰的数字需要进入到stack2 故先处理stack2的数据
    // 每次循环 筛选完stack2后保留的都是大于等于cur的数 同时从stack1中弹出的数需要进入stack2
    // 这些数均小于cur 为了维护stack2的单调性 需要将stack1的数在stack2原有数之前进行访问
    // 同时stack1弹出数的顺序是升序 直接push会变成降序 这里采用一个媒介来对stack1弹出的数进行逆序
    public static int[] secondGreaterElement(int[] nums) {
        ...
        for (int i = 0; i < n; i++) {
            int cur = nums[i];
            while (!stack2.isEmpty() && nums[stack2.peek()] < cur)
                ans[stack2.pop()] = cur;
            while (!stack1.isEmpty() && nums[stack1.peek()] < cur)
...
### LC.2415. 反转二叉树的奇数层 -2023-12-15

```java
    // lc.2415. 反转二叉树的奇数层
    // 层序遍历 判断下一层是否为奇数层 交换节点值而不是引用
...
### LC.2276. 统计区间中的整数数目 -2023-12-16

```java
    // lc.2276. 统计区间中的整数数目
    // 树状数组线段树 或 TreeMap维护区间
    // 插入区间时需要与现有的区间进行合并 cnt需要剔除重复部分
    // 剔除重复部分采用 删除上一个区间的计数 然后添加合并区间的计数(有重复部分)
    class CountIntervals {
        ...
        public void add(int left, int right) {
            ...
            while (last != null && last.getValue() >= left){
                // 有重复部分
                int l = last.getKey(), r = last.getValue();
                left = Math.min(l, left);
                right = Math.max(right, r);
                cnt -= r - l + 1;
                map.remove(l);
                last = map.floorEntry(right);
            }
            cnt += (right - left + 1);
            map.put(left, right);
        }
...
### LC.746. 使用最小花费爬楼梯 -2023-12-17

```java
    // lc.746. 使用最小花费爬楼梯
    // dp 爬到当前层数是从前一层或前两层处到达 计算dp[n]
    public int minCostClimbingStairs(int[] cost) {
        int n = cost.length;
        int dp1 = 0;
        int dp2 = 0;
        for (int i = 2; i <= n; i++) {
            int cst = Math.min(dp1 + cost[i-1], dp2 + cost[i-2]);
            dp2 = dp1;
            dp1 = cst;
        }
        return dp1;
...
### LC.162. 寻找峰值 -2023-12-18

```java
    // lc.162. 寻找峰值
    // 最简单的想法 遍历即可
    // O(log n) 考虑二分 可能会有多个峰值 只需要找到一个即可
    // 两侧取值为 -∞  且任意两数不相等 那么必然存在一个峰值
    // 需要的就是维护一个/ ~ \ 这样的区间
    // 数学证明:
    // 已知区间端点处呈上升趋势 若中间只有一个点 即为峰值
    // 若有多个节点 由于任意节点的值不相等 那么节点值的变化趋势只有上升和下降
    // 从端点开始 若下一个节点大于当前节点 那么上升的趋势不会改变
    //                       若下一个节点小于当前节点 那么当前节点即为峰值
    // 1.上升趋势不改变 由于末端存在下降趋势 那么末节点为峰值
    // 2.上升趋势改变 则转折点为峰值
    // 综上: 对于 / ~ \ 这样的区间 必然存在峰值
    // 同理可得 对于 \ ~ / 这样的区间 必然存在谷值
...
### LC.1901. 寻找峰值 II -2023-12-19

```java
    // lc.1901. 寻找峰值 II
    // 最简单的想法 遍历 m * n
    // 对于二维网格通常做法: 二分 转化为图 dp
    // 直接按 lc.162. 寻找峰值 的思路对于每一行找到一个峰值再进行判断这种方法不可行
    // 由于上述方法只要求找到峰值 可能会错过真正的峰值
    // 实际上找到峰值 全网格中最大的值一定是峰值 但mat无序 没有好的算法
    // 向中间的上升趋势 + 各个节点值不相等 必然会产生峰值(极大值)
    // 但是现在有4个方向 按照二分的想法只能控制两个方向的聚拢 (例如竖直方向)
    // 为了能够继续控制另外两个方向 可以选择该方向上的最值 (水平方向)
    // 最值本身距离水平方向的聚拢趋势 同时最值还能继承竖直方向上的聚拢趋势
    // mat[i][maxj] > mat[i-1][maxj'] > mat[i-1][]
    public int[] findPeakGrid(int[][] mat) {
        ...
        while (l <= r) {
            ...
            if (mid > 0 && mat[mid][maxIdx] < mat[mid - 1][maxIdx]) {
                r = mid - 1;
            } else if (mid < m - 1 && mat[mid][maxIdx] < mat[mid + 1][maxIdx]) {
                l = mid + 1;
            } else {
...
### LC.2866. 美丽塔 II -2023-12-21

```java
    // lc.2866. 美丽塔 II
    // 最简单的想法就是分别取每个位置作为顶 然后尽可能大的选取其他值
    // 选取后的取值就是构造非减和非增序列
    // sumH[i] = prefix[i] + suffix[i] - maxHeights.get(i)
    // prefix[i] 表示为 i及i之前的元素能构成的最大非降序和
    // suffix[i] 表示为 i及i之后的元素能构成的最大非升序和
    // 为什么不定义为 i之前 或 i之后 的元素呢
    // 如果这样定义 prefix[j] = prefix[i] + (i到j-i的数据之和)
    // 但实际上由于maxHeights.get(i) < maxHeights.get(j) 是无法合并到[i+1,j-1]的数据里的
    // 可以认为这里是为了将能取相同值的元素合并在一起
